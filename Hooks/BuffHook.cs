    ...
    private static void Prefix(ModifyUnitStatBuffSystem_Spawn __instance)
    {
        ...
        foreach(var entity in entities)
        {
            ...
            if (GUID.Equals(Database.Buff.Buff_VBlood_Perk_Moose))
            {
                ...
                {
                    StatType = UnitStatType.MaxHealth,
                    Value = RebirthSystem.getLevel(Data.PlatformId) * 0.65f + 1000f,
                    ModificationType = ModificationType.Add,
                    Id = ModificationId.NewId(0)
                });
                ...
                int level;
                if (Database.rebirths.TryGetValue(Data.PlatformId, out level)) 
                {
                    foreach (ModifyUnitStatBuff_DOTS modifyUnitStatBuff_DOTS in RebirthSystem.GetBonusStats(level))
                    {
